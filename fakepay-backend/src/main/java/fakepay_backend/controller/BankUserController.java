...
import fakepay_backend.service.BankUserService;
...
public class BankUserController {
    ...
    public ResponseEntity<fakepay_backend.dto.BankBalanceResponse> getBankBalance(@PathVariable String phoneNo) {
        fakepay_backend.dto.BankBalanceResponse response = bankUserService.getBankUserBalance(phoneNo);
        return ResponseEntity.ok(response);
    }

    @PostMapping("/check-balance")
    public ResponseEntity<?> checkBalanceWithPin(@RequestBody fakepay_backend.dto.CheckBalanceRequest request) {
        try {
            fakepay_backend.dto.BankBalanceResponse response = bankUserService.verifyPinAndGetBalance(request);
            return ResponseEntity.ok(response);
